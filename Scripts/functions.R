...
#'
#' Output:
#' @returns A list of the inputted data frames containing the same non-NA values as before, only
#' with all rows and columns that consist of NAs only having been removed. Can then be used to
#' assign them as variables in a specified environment, using the list's names as variable names.
remove_NA <- function(...) {
  ...
  # Automatically collect the names of input arguments in '...' via 'substitute(list(...))', then
  # remove the list information of the substitute object via '[-1]' in order to get the names of the
  # data frames alone and lastly convert them to character strings via 'deparse'
...
#' FUNCTION 2:
#' @description
#' The function 'group_data' groups the data frame values according to a certain indicator by
#' comparing the indicator's average value with self-defined quantile intervals and assigning the
#' corresponding self-defined group labels.
#'
#' Inputs:
#' @param df A data frame containing at least the columns 'Country Name', 'Country Code', 'Series
#' Name', 'Series Code' and 'average', further columns are possible.
#' @param grouping_by A single string being an entry of the column 'Series Code' specifying the
#' indicator to group the averages by.
...
#' according to scheme: 'cat_' + column_new (e.g. column_new = 'population' --> 'cat_population').
...
#'
#' Output:
#' @returns A data frame of the columns 'Country Name', 'Country Code' and the newly created column
...
#' FUNCTION 3:
#' @description
#' The function 'align_colnames' aligns the column names of several data frames by finding character
#' column names containing single dots and replacing the dots by single spaces.
#'
#' Inputs:
#' @param ... One or more data frames of no further specification.
#'
#' Output:
#' @returns A list of the inputted data frames with the same columns as before only with new column
#' names for all character columns containing single dots, with the single dots being replaced by a
#' space. Can then be used to assign them as variables in a specified environment, using the list's
...
align_colnames <- function(...) {
  ...
  # Align the data frames column names by replaceing dot between words with space
  require(dplyr)
  dfs_cleaned <- lapply(dfs, function(df) {
    df <- df %>%
...
#' FUNCTION 4:
#' @description
#' The function 'remove_metadata' searches a data frame for inputted metadata in the 'Country Name'
#' column and deletes the metadatas' rows.
#'
#' Inputs:
#' @param metadata_source A single string with the source of the data set, that is being edited.
#' @param metadata_time A single string with the information of the last changes done to the data
#' set, that is being edited.
#' @param ... One or more data frames containing column 'Country Name', other columns are possible.
#'
#' Output:
#' @returns A list of the inputted data frames after being adjusted. Can then be used to assign them
#' as variables in a specified environment, using the list's names as the variable names.
...
#' FUNCTION 5:
#' @description
#' The function 'translate' takes data frames and applies a self-defined translation mapping for
#' entries in a selected column and changes those entries according to the mapping.
#'
#' Inputs:
#' @param mapping A named character vector of any length describing the translation that is supposed
#' to happen. The names are the entries that will be checked for existence in the selected column.
#' In case of existence, the character entry by that name will be inserted instead of the name.
#' @param column A single string naming the column in which the tranlation mapping shall be done.
#' @param ... One or more data frames containing column 'column', other columns are possible.
#'
#' Output:
#' @returns A list of the inputted data frames with the translated values for the mapping names that
#' could be found in the selected column. Can then be used to assign them as variables in a
...
#'
#' Inputs:
#' @param ... One or more data frames containing - among others - the columns 'average' and 'Series
#' Name' as well as several columns with names representing different years in the format 'YYYY'.
#'
#' Output:
#' @returns A list of the inputted data frames without the columns 'average' and 'Series Name' and
#' columns of year format (i.e. YYYY) of numeric type. Can then be used to assign them as variables
#' in a specified environment, using the list's names as the variable names.
prepare_join <- function(...) {
  ...
  # Delete the column 'average' and 'Series Name' and ensure that all year columns are numeric to
  # to avoid data type issues while merging
  require(dplyr)
  dfs_cleaned <- lapply(dfs, function(df) {
    df <- df %>% 
