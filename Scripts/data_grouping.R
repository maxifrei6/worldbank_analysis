...
data_grouped_tob <-
  group_data(df = data_processed,
             ...
             column_new = "tob_usage",
             quantile_labels = c("Very Low", "Low", "Medium", "High", "Very High"))

# 4. Grouping by population size
data_grouped_pop <-
  group_data(df = data_processed,
             grouping_by = "SP.POP.TOTL",
             column_new = "population_size",
             quantile_labels = c("Low", "Medium", "High"))

# 5. Grouping by population density
data_grouped_pop_density <-
  group_data(df = data_processed,
             grouping_by = "EN.POP.DNST",
             column_new = "population_density",
             quantile_labels = c("Low", "Medium", "High"))

# 5. Grouping by income levels
data_grouped_income_levels <-
  group_data(df = data_processed,
             grouping_by = "NY.ADJ.NNTY.PC.CD",
             column_new = "income_level",
             quantile_labels = c("Low", "Medium", "High"))
...
# Merge all groups
data_grouped <- 
  left_join(data_grouped_alc,data_grouped_edu, by = c("Country Name", "Country Code")) %>%
