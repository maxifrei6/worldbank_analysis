...
data_co2 <- data_co2 %>%
  mutate(across(
    .cols =
      matches("^X(2000|20[0-1][0-9]|2020|2021)..YR(2000|20[0-1][0-9]|2020|2021)\\.$"),
    .fns = ~ gsub(pattern = "\\.", replacement = "", .)))

# If the data entries start with zero, the remaining digits behind the zero are decimal
# places. Therefore, put a dot behind the zero if it is a leading zero.
data_co2 <- data_co2 %>%
  mutate(across(
    .cols =
      matches("^X(2000|20[0-1][0-9]|2020|2021)..YR(2000|20[0-1][0-9]|2020|2021)\\.$"),
...
# For all other data entries, insert a dot after fourth digit from the right if number has
# more than four digits or after third digit from the right if number has at most four
# digits, thereby converting it to decimal number.
data_co2 <- data_co2 %>%
  mutate(across(
    .cols =
      matches("^X(2000|20[0-1][0-9]|2020|2021)..YR(2000|20[0-1][0-9]|2020|2021)\\.$"),
...
data_sociometrics <-
  read_excel(here("Data", "Raw", "Worldbank1.xlsx"),
             sheet = 1, col_names = TRUE, na = "..")

# TODO: CREATE DICTIONARY/LIST FROM SHEET 2 TO MATCH ABBREVIATED METRICS WITH DESCRIPTION

# Delete columns or rows if they consist of only NAs. Apply 'remove_NA' to all interested
# data sets using 'list2env' to automatically update the inputted variables without manual
# assignment.
...
# Remaining column names in data_geo and data_co2 aligning with data_sociometrics, i.e.
# replacing dot between words with space. Apply 'align_colnames' to all interested data
# sets using 'list2env' to automatically update the inputted variables without manual
# assignment.
list2env(align_colnames(data_geo, data_co2, data_sociometrics), envir = .GlobalEnv)

# Removing metadata rows by applying 'remove_metadata' to all interested data sets using
# 'list2env' to automatically update the inputted variables without manual assignment.
list2env(
  remove_metadata(metadata_source = "Data from database: World Development Indicators",
                  metadata_time = "Last Updated: 10/24/2024",
                  data_geo, data_co2, data_sociometrics),
  envir = .GlobalEnv)
...
# Define the translation mapping for the 'Country Name' column to German for plot
# visualization purposes at later point.
...
# Change the 'Country Name' column by applying 'translate' to all interested data sets
# using 'list2env' to automatically update the inputted variables without manual
# assignment.
list2env(translate(
  translation_mapping, "Country Name", data_geo, data_co2, data_sociometrics),
  envir = .GlobalEnv)
...
# Delete column 'average' and 'Series Name' to avoid data type issues and ensure all year
# columns are numeric to avoid merge errors by applying 'prepare_join' to all interested
# data sets using 'list2env' to automatically update the inputted variables without manual
# assignment.
...
# Convert the column 'Year' to data type integer.
data_merged$Year <- as.integer(data_merged$Year)
