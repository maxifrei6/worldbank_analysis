...
data_tob_per_year <- data_tob %>%
  ...
  mutate(cat_tob_usage_per_year = 
           group_data_year(df = data_tob, grouping_by = "SH.PRV.SMOK", year = Year, column_new = "tob_usage_per_year", quantile_labels = c("Sehr gering", "Gering", "Mittel", "Viel", "Sehr Viel"))$cat_tob_usage_per_year)
```
<br>
...
data_tob_per_year %>%
  ...
  labs(
    x = "Anteil der tabakkonsumierenden Erwachsenen in %",
    y = "BIP pro Kopf in $",
    title = "Tabakkonsum vs. BIP alle Jahre jährlich gruppiert",
    colour = "relativer Tabakkonsum"
  ) +
  geom_smooth(aes(group = 1), method = "lm", se = FALSE) +
  scale_colour_brewer(palette = "Set2")
...
```
<br>
und nun barplot facettiert nach jahr, x: gruppiert nach tabakdurchschnittskonsum und dann bip geplottet
<br>
```{r, echo = FALSE, warning = FALSE, message = FALSE, fig.align = "left"}
data_tob %>%
  group_by(cat_tob_usage) %>%
