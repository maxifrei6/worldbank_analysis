...
data_tob %>%
  filter(Year == 2020) %>%
  ggplot(aes(`Country Name`, SH.PRV.SMOK, colour = tobacco_usage)) +
...
data_tob %>%
  filter(Year == 2020) %>%
  group_by(tobacco_usage) %>%
  ggplot(aes(SH.PRV.SMOK, NY.GDP.PCAP.PP.KD, colour = tobacco_usage)) +
  geom_point(size = 3, alpha = 0.7) +
  labs(
...
```{r, echo = FALSE, warning = FALSE, message = FALSE, fig.align = "left"}
data_tob %>%
  group_by(Year, tobacco_usage) %>%
  summarise(
    NY.GDP.PCAP.PP.KD = mean(NY.GDP.PCAP.PP.KD, na.rm = TRUE),
    SH.PRV.SMOK = mean(SH.PRV.SMOK, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  ggplot(aes(Year, NY.GDP.PCAP.PP.KD, colour = tobacco_usage)) +
  geom_point(size = 3, alpha = 0.7) +
  geom_line(aes(group = tobacco_usage)) +
  theme(axis.text.x = element_text(
    angle = 45,
    hjust = 1,
    size = 6
