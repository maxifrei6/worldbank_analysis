...
<br>
<br>
```{r tobacco Leonie setup, message = FALSE, echo = FALSE, warning = FALSE}
...
data_tob_per_year_format %>%
 ...
 #  ))
```
```{r tobacco leonie graph 1, echo = FALSE, warning = FALSE, message = FALSE, fig.align = "left"}
tobacco_leonie_graph_1 <- data_tob_per_year_format %>%
  group_by(cat_tob_usage_per_year) %>%
  ggplot(aes(x = cat_tob_usage_per_year, y = NY.GDP.PCAP.PP.KD)) +
  geom_col() +
  facet_wrap( ~ Year) +
