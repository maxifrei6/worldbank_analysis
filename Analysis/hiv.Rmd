...
```
<br> 
Auffällig ist, dass es überhaupt keine Daten für Arube zu geben scheint, daher wird dieses Land im folgenden aus dem Datensatz entfernt, den wir für diese Fragestellung brauchen.
<br>
```{r, echo=FALSE, warning = FALSE, message = FALSE, fig.align = "left"}
data_hiv <- data_hiv %>%
...
```
<br>
New try: Now we take a look at all years, facetted by cat_education, all countries each a line
<br>
```{r, echo = FALSE, warning = FALSE, message = FALSE, fig.align = "left"}
data_hiv %>%
  ggplot(aes(SL.TLF.BASC.ZS, SH.DYN.AIDS.ZS, colour = `Country Name`)) +
  geom_point(size = 3, alpha = 0.7) +
  geom_line(aes(group = `Country Name`)) +
  facet_wrap(~cat_education)
  labs(
    title = "HIV Prävalenz über alle Jahre facettiert nach relativen Bildungsstand der Arbeiterschaft",
    x = "Anteil grundlegend gebildeter Arbeitskräfte",
    y = "HIV Prävalenz",
    colour = "Land"
  )  +
...
<br>
Here we see clearly that some countries change their education group to belong to over the years. Therfore we should try to group the data per year later.
But first the next try: Now we take a look at all years, facetted by cat_education, all countries each a line
<br>
```{r, echo = FALSE, warning = FALSE, message = FALSE, fig.align = "left"}
data_hiv %>%
  ggplot(aes(Year, SH.DYN.AIDS.ZS, colour = `Country Name`)) +
  geom_point(size = 3, alpha = 0.7) +
  geom_line(aes(group = `Country Name`)) +
  facet_wrap(~cat_education)
  labs(
