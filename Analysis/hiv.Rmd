...
plot_alc_hiv_grouped_dotplot <- data_alc_per_year %>%
  ggplot(aes(SH.ALC.PCAP.LI, SH.DYN.AIDS.ZS)) +
  geom_point(aes(fill = cat_alc_per_year), size = 4, alpha = 0.5, shape = 21) +
  scale_fill_viridis_d(name = "relativer Alkoholkonsum", direction = -1, option = "plasma", guide = guide_legend(reverse = TRUE)) +
  geom_smooth(aes(group = 1), method = "lm", se = FALSE, size = 0.8, colour = "black") +
  labs(
...
plot_edu_hiv_grouped_dotplot <- data_edu %>%
  ...
  ggplot(aes(SL.TLF.BASC.ZS, SH.DYN.AIDS.ZS)) +
  geom_point(aes(fill = cat_edu_per_year), size = 5, alpha = 0.7, shape = 21) +
  scale_fill_viridis_d(name = "relativer Anteil an Arbeitenden\nmit grundlegender Bildung", option = "plasma", direction = -1, guide = guide_legend(reverse = TRUE)) +
  theme_bw(base_size = 21) +
  labs(
...
plot_edu_hiv_boxplot <- data_edu %>%
  ...
  theme_bw(base_size = 20) +
  labs(
    x = "Arbeitende mit grundlegender Bildung [%]",
