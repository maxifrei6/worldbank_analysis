...
<br>
First we want to look at the existing data about the relevant variables.
<br>
...
```
<br>
let's look at the grouping of this data in general.
<br>
...
```
<br>
We can see some overlapping of the different groups, which could arise from plotting the data of all years. We will take a closer look in the following.
<br>
First, we plot the grouping of our data by alcohol consumption of the most recent data in Year 2020 to get familiar with our data set.
<br>
...
```
<br>
Now we view only one dot per category of relative alcohol consumption by calculating the mean.
<br>
...
```
<br>
We want to look at the ratio per year and are therefore now faceting over the year with the general grouping.
<br>
...
```
<br>
Only very slight fluctuations in the distribution are recognizable here. Now let's come back to the idea of annual grouping. 
<br>
...
```
<br>
Here is a plot of the new grouping:
<br>
...
```
<br>
Now we only plot one point per category.
<br>
...
```
<br>
Combined into one plot, the data would result in the following:
...
data_alc_per_year %>%
  ...
  geom_smooth(aes(group = 1), method = "loess", size = 0.8, span = 1.05) +
  scale_colour_brewer(palette = "Set2")
```
<br>
However, in order to include the scattering of the values, the same data is shown in a box plot:
<br>
...
```
<br>
Here is the plot again, which was caused by the general grouping. For comparison:
<br>
...
```
<br>
In order to take a closer look at the grouping, we plot the data including the alcohol consumption variable:
<br>
...
```
<br>
Here we can now see the actual improved grouping very clearly.
<br>
If we now want to look at the plots faceted by year with the new grouping, it looks like this:
<br>
...
```
<br>
Here is another plot from earlier that we would like to analyze further.
<br>
We also don’t want to present the data in an oversimplified manner. You should see how the graph is composed. The following graphs are quite good for this. So here's a plot with the same statement about the averages for all years with data regrouped every year. One point per year is displayed here and a polynomial (LOESS method, span = 1.05) is placed over it.
<br>
...
```
<br>
What is difficult here is that the mean value calculation can distort the data.
<br>
Here, an average point is shown per country over all years and a polynomial (method = "loess", span = 1.05) is placed over it.
<br>
...
```
<br>
The problem here is that countries that have belonged to several categories over the years claim several data points. 
This gives these countries more weight.
<br>
For comparison here a plot with all data points. Again with a smooth line (method = "loess", span = 1.05).
<br>
