...
presentation_education_plot4 <- ggplot(data_education, aes(x = SE.TER.ENRL.TC.ZS, y = SL.TLF.BASC.ZS)) +
...
# German plot for presentation: Scatter plot: pupil-teacher ratios vs labor force with basic education
pres_updated_education_plot4 <- ggplot(data_education, aes(x = SE.TER.ENRL.TC.ZS, y = SL.TLF.BASC.ZS)) +
  geom_point(aes(color = cat_population_density), size = 3.5, alpha = 0.6) +
  geom_smooth(method = MASS::rlm, color = "black", alpha = 0.2) +
  labs(
    title = "Schüler-Lehrer-Relation vs. Grundbildung",
...
pres_updated_education_plot6 <- ggplot(data_education, aes(x = SE.TER.ENRL.TC.ZS, y = NY.GDP.PCAP.PP.KD, fill = cat_population_density)) +
  geom_point(size = 3.5, shape = 21, alpha = 0.4) +
