...
data_with_cor <- data_education %>%
  left_join(cor_matrix, by = "cat_income_level")

# Update geom_text with the corrected label formatting
...
# Calculate Spearman correlation for each population density category
cor_matrix <- data_education %>%
  group_by(cat_population_density) %>%
  summarise(
...
# Merge correlations back into the original data for placement
data_with_cor <- data_education %>%
  left_join(cor_matrix, by = "cat_population_density")

# Create the scatter plot with the correlations
p5 <- ggplot(data_with_cor, aes(x = SE.TER.ENRL.TC.ZS, y = SL.TLF.BASC.ZS)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = MASS::rlm, se = FALSE, color = "black") +
  facet_wrap(~ cat_population_density) +
  labs(
    title = "Pupil-Teacher Ratio vs Labor Force with Basic Education by Population Density",
    x = "Pupil-Teacher Ratio",
    y = "Labor Force with Basic Education (%)"
  ) +
  theme_bw() +
...
ggplot(data_education, aes(x = NY.GDP.PCAP.PP.KD, y = SL.TLF.BASC.ZS, fill = cat_population_density)) +
  geom_point(shape = 21, alpha = 0.4) +
  geom_smooth(aes(color = cat_population_density), method = MASS::rlm, se = FALSE) +
  scale_x_log10() +  # Apply log10 transformation to the x-axis
  labs(
    title = "GDP per Capita (Log Scale) vs Labor Force with Basic Education",
