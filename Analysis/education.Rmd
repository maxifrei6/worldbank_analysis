...
# Scatter plot: Central government debt vs labor force with basic education
scatter_plot <- ggplot(data_education, aes(x = GC.DOD.TOTL.GD.ZS, y = SL.TLF.BASC.ZS, color = cat_income_level)) +
  geom_point(alpha = 0.7, size = 3) +
  geom_smooth(method = "lm", se = FALSE, color = "black") +
  labs(
    title = "Central Government Debt vs Labor Force with Basic Education",
...
ggsave(
  filename = "/Users/maximilian/Desktop/wb_local/Results/Figures/education_scatter_income_lvl.png",
  plot = scatter_plot,
...
contingency_table_1 <- data_education %>%
  mutate(
    data_pair_available = !is.na(GC.DOD.TOTL.GD.ZS) & !is.na(SL.TLF.BASC.ZS)
  ) %>%
  group_by(cat_income_level, cat_population_size) %>%
  summarise(
    total_rows = n(),
    available_pairs = sum(data_pair_available),
    proportion_available = round(available_pairs / total_rows, 2)
  ) %>%
  ungroup() %>%
  select(cat_income_level, cat_population_size, proportion_available) %>%
  pivot_wider(
    names_from = cat_population_size,
    values_from = proportion_available,
    values_fill = 0 # Fill missing combinations with 0
  )

contingency_table_2 <- data_education %>%
  mutate(
    data_pair_available = !is.na(GC.DOD.TOTL.GD.ZS) & !is.na(SL.TLF.BASC.ZS)
  ) %>%
  group_by(cat_income_level, cat_population_size) %>%
  ...
  ungroup() %>%
  select(cat_income_level, cat_population_size, available_pairs) %>%
  pivot_wider(
    names_from = cat_population_size,
    values_from = available_pairs,
    values_fill = 0 # Fill missing combinations with 0
  )
...
ggplot(data_education, aes(x = SE.TER.ENRL.TC.ZS, y = SL.TLF.BASC.ZS, color = cat_population_density)) +
  geom_point(alpha = 0.7, size = 3) +
  geom_smooth(method = "lm", se = FALSE, color = "black") +
  labs(
    title = "Pupil-Teacher Ratio vs Labor Force with Basic Education",
...
```{r, message=FALSE, warning=FALSE, echo=FALSE}
# Scatter plot: pupil-teacher ratios vs labor force with basic education
ggplot(data_education, aes(x = SE.TER.ENRL.TC.ZS, y = SL.TLF.BASC.ZS, color = cat_population_size)) +
  geom_point(alpha = 0.7, size = 3) +
  geom_smooth(method = "lm", se = FALSE, color = "black") +
  facet_wrap(~ cat_population_size) +
  labs(
    title = "Pupil-Teacher Ratio vs Labor Force with Basic Education",
    x = "Pupil-Teacher Ratio",
    y = "Labor Force with Basic Education (%)",
...
ggplot(data_education, aes(x = NY.GDP.PCAP.PP.KD, y = SL.TLF.BASC.ZS, color = cat_income_level)) +
  geom_point() +
