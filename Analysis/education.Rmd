...
base_plot_1 <- ggplot(available_data_pairs, aes(x = reorder(`Country Name`, -available_pairs), y = available_pairs)) +
  geom_bar(stat = "identity") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))

# English Plot: Available Data Pairs per Country
english_plot_1 <- base_plot_1 +
  labs(
...
pres_updated_education_backup1 <- base_plot_1 +
  labs(
...
base_plot_2 <- ggplot(data_education_2, 
...
pres_updated_education_backup2 <- base_plot_2 +
  labs(
...
english_plot_3 <- ggplot(data_education, aes(x = GC.DOD.TOTL.GD.ZS, y = SL.TLF.BASC.ZS)) +
...
presentation_education_plot1 <- german_plot_1 +
...
pres_updated_education_plot1 <- german_plot_1 +
...
english_plot_7 <- ggplot(data_with_cor, aes(x = GC.DOD.TOTL.GD.ZS, y = SL.TLF.BASC.ZS)) +
  geom_point(alpha = 0.6) +
...
presentation_education_plot2 <- english_plot_7 +
  geom_point(size = 3.5, alpha = 0.6) +
...
# German plot for presentation (Updated)
pres_updated_education_plot2 <- presentation_education_plot2 +
...
german_correlation <- tidy_correlation %>%
  mutate(
...
presentation_education_plot3 <-
  ggplot(german_correlation, aes(x = cat_income_level, y = cat_population_size, fill = correlation)) +
  geom_tile(color = "white") +
...
pres_updated_education_plot3 <- presentation_education_plot3 +
  labs(title = "Zentralstaatliche Verschuldung und Grundbildung")
...
german_plot_2 <- ggplot(data_education, aes(x = SE.TER.ENRL.TC.ZS, y = SL.TLF.BASC.ZS)) +
...
presentation_education_plot4 <- german_plot_2 +
  geom_smooth(method = MASS::rlm, se = FALSE, color = "black") +
  theme(
        legend.text = element_text(size = 14),
        legend.title = element_text(hjust = 0.5, size = 14),
...
# German plot for presentation: Scatter plot: pupil-teacher ratios vs labor force with basic education
pres_updated_education_plot4 <- german_plot_2 +
  geom_smooth(method = MASS::rlm, color = "black", alpha = 0.2) +
  labs(
...
english_plot_9 <- ggplot(data_with_cor, aes(x = SE.TER.ENRL.TC.ZS, y = SL.TLF.BASC.ZS)) +
  geom_point(alpha = 0.6) +
...
presentation_education_plot5 <- german_plot_4  +
  geom_smooth(method = MASS::rlm, se = FALSE, color = "black")

# Updated German plot for presentation
pres_updated_education_plot5 <- german_plot_4  +
  geom_smooth(method = MASS::rlm, color = "black", alpha = 0.2) +
...
english_plot_11 <- ggplot(data_education, aes(x = SE.TER.ENRL.TC.ZS, y = NY.GDP.PCAP.PP.KD, fill = cat_population_density)) +
...
english_plot_12 <- english_plot_11 +
