...
ggplot(data_agriculture, aes(x = factor(Year), y = factor(`Country Name`, levels = country_order_by_CO2variance), fill = log10(EN.GHG.CO2.MT.CE.AR5 + 1))) +
  ...
  scale_fill_viridis_c(option = "plasma", name = "CO2 Emissionen pro Kopf\n[Mt] (Log10)",
                       direction = -1,
...
# Country-faceted line plot displaying the development of agricultural land [%] ordered by ascending mean [%]
...
# Initial comparison via scatter plot of agricultural land [%] and CO2 emissions per capita [Mt]
...
# Logarithmic CO2 emissions by country, average agricultural land [%] for color and faceting order
ggplot(data_agriculture, aes(x = Year, y = log10(EN.GHG.CO2.MT.CE.AR5 + 1))) +
  ...
  ggtitle("Zusammenhang zwischen CO2 Emissionen pro Kopf und landwirtschaftlicher Nutzfläche nach Ländern") +
  scale_x_continuous(name = "Jahr", breaks = seq(2000, 2020, by = 5),
                     minor_breaks = seq(2000, 2021, by = 1)) +
