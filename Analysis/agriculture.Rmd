...
ggplot(data_agriculture, aes(x = factor(Year), y = factor(`Country Name`, levels = country_order_by_CO2variance), fill = log10(EN.GHG.CO2.MT.CE.AR5 + 1))) +
  geom_tile(col = "black") +
...
ggplot(data_agriculture,
       aes(x = log10(EN.GHG.CO2.MT.CE.AR5 + 1), y = factor(`Country Name`, levels = country_order_by_CO2variance))) +
  geom_boxplot() +
  ggtitle("Streuungszerlegung der CO2 Emissionen\npro Kopf bezüglich Ländern") +
  scale_x_continuous(name = "Logarithmierte CO2 Emissionen pro Kopf [Mt]",
                     breaks = log10(c(0, 10, 100, 1000, 10000) + 1),
                     labels = c("0", "10", "100", "1.000", "10.000")) +
  scale_y_discrete(name = "Land") +
  theme_bw() +
...
# Radar chart for first overview of the interaction of the three variables - bounded by country barriers
# set by the countries we are observing.
data_agriculture_radar <- data_agriculture %>%
  mutate(EN.GHG.CO2.MT.CE.AR5 = log10(EN.GHG.CO2.MT.CE.AR5 + 1)) %>%
  rename(log10_EN.GHG.CO2.MT.CE.AR5 = EN.GHG.CO2.MT.CE.AR5)

data_agriculture_radar <- data_agriculture_radar %>%
  group_by(`Country Name`) %>%
...
ggradar(data_agriculture_radar,
        axis.label.size = 3,
        grid.label.size = 4,
  ...
  ggtitle("Indikatordaten im Zeitraum 2000 bis 2021",
          subtitle = "Mittlere Indikatordaten pro Land pro Indikator") +
  theme_minimal() +
  scale_color_brewer(name = "Indikatoren\n(auf 0-100 % der beobachteten Länder)",
                     palette = "Set2",
                     labels = c("AG.SRF.TOTL.K2" = "Landfläche",
                                "AG.LND.AGRI.ZS" = "Landwirtschaftliche Nutzfläche",
                                "log10_EN.GHG.CO2.MT.CE.AR5" = "Logarithmierte CO2-Emissionen pro Kopf")) +
...
# TODO: Change the readRDS(...) loading to non-hardcode RDS file in Data/Processed - DONE :)
# TODO: Change grouping of data to read the correct RDS file from Processed instead of grouping here!
# Source the functions script using `here`
source(here("Scripts", "functions.R"))
# Load the grouped dataset using `here`
data_grouped <- readRDS(here("Data", "Processed", "data_pregrouped.RDS"))
