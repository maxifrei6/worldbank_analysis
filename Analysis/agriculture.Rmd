...
ggplot(data_agriculture, aes(color = AG.LND.AGRI.ZS)) +
...
ggplot(data_agriculture_surfaceChange, aes(x = Year)) +
  ...
  labs(title = "",  # TODO: INSERT TITLE
       x = "Jahr", y = "Absolute Änderung in Landfläche") +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5),
...
```{r surface area changeless countries: relative, echo = FALSE, warning = FALSE, message = FALSE}
...
####################### TODO: Change the data_grouped_agr loading to readRDS(...) of correct RDS file in Data/Processed
data_grouped <- data_grouped_agr
...
ggplot(data_surface, aes(x = log10(EN.GHG.CO2.MT.CE.AR5 + 1), y = AG.LND.AGRI.ZS)) +
  ...
  labs(title = "Zusammenhang zwischen CO2 Emissionen pro Kopf\nund landwirtschaftlicher Nutzfläche",
       x = "Logarithmierte CO2 Emissionen pro Kopf [mt]", y = "Landwirtschaftliche Nutzfläche [%]") +
  scale_color_brewer(palette = "Set2") +  # TODO: SHOULDN'T THIS BE A SEQUENTIAL COLORGRADING -> WHICH PALETTE?
  # scale_x_log10(name = "CO2 Emissionen pro Kopf",
  #               breaks = log10(c(0, 10, 100, 1000, 10000, 100000) + 1),
  #               labels = c("0", "10", "100", "1.000", "10.000", "100.000")) +
  # scale_x_log10(breaks = trans_breaks("log10", function(x) 10 ^ x),
