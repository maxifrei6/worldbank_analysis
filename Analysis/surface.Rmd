...
ggplot(data_surface_change, aes(x = Year)) +
  ...
  scale_y_continuous(
    name = expression(Absolute ~ Änderung ~ "in" ~ Landfläche ~ "[" ~ km^2 ~ "]")
    ) +
...
(plot_changingObs_left | plot_changingObs_right) + 
  plot_annotation(title = paste0("Landfläche in Ländern mit Veränderungen",
...
ggplot(data_surface,
  ...
  scale_x_continuous(
    name = expression(Landfläche ~ "[" ~ km^2 ~ "]" ~ (Log10)),
